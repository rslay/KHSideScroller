...
const UPSCALE_MULTIPLIER = 2
onready var root = get_tree().get_root()
onready var base_size = Vector2(832 * UPSCALE_MULTIPLIER, 736 * UPSCALE_MULTIPLIER)
...
func _on_screen_resized():
	...
	var scale_w = max(int(new_window_size.x / base_size.x), UPSCALE_MULTIPLIER)
	var scale_h = max(int(new_window_size.y / base_size.y), UPSCALE_MULTIPLIER)
